...
## 自定义类加载器
[MyTest16.java](jvm_lecture/src/main/java/org/iproute/jvm/classloader/MyTest16.java)
...
- `com.mysql.jdbc.Driver` 被主动调用，要执行初始化的操作
    - 关注一下 `com.mysql.jdbc.Driver` 类中的静态代码块
        ```
       /**
...
- `com.mysql.jdbc.Driver` 类中的静态代码块显示调用了 `java.sql.DriverManager`,所以要对 `DriverManager` 进行初始化
    - 关注一下  `java.sql.DriverManager` 类中的静态代码块
        ```
        static {
            loadInitialDrivers();
...
- 关键方法 `line661: isDriverAllowed(aDriver.driver, callerCL)`
    ```
    ...
    - **关键**：`result = ( aClass == driver.getClass() ) ? true : false;`
